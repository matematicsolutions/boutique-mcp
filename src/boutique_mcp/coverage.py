...
SOURCE = 'the MateMatic Boutique catalog (catalog.json, mirrored from matematicsolutions.com)'
...
_FAMILIES: list[dict] = [{'name': 'Boutique catalogue entries', 'tool': 'boutique_search / boutique_get / boutique_whats_new'}]

_GAPS: list[dict] = [{'id': 'BTQ-001', 'family': 'Boutique catalogue entries', 'missing': 'The catalogue lists what MateMatic ships or curates; it is not a directory of all legal MCP servers or skills.', 'fallback': 'Search the MCP registry or GitHub for tools outside this catalogue.'}, {'id': 'BTQ-002', 'family': 'Boutique catalogue entries', 'missing': "Entries describe a tool's scope, not its live health - a listed connector can still be down or rate-limited.", 'fallback': 'Call the connector itself, or its own coverage tool, before relying on it.'}, {'id': 'BTQ-003', 'family': 'Boutique catalogue entries', 'missing': 'The catalogue can be served from a bundled snapshot when the network is unavailable, in which case it lags the site.', 'fallback': 'Check the provenance stamp on any response; when stale is true, re-fetch.'}]
