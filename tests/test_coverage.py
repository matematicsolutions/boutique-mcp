...
from boutique_mcp.coverage import build_coverage
from boutique_mcp.server import boutique_coverage


def _coverage():
    fn = getattr(boutique_coverage, "fn", boutique_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("BOUTIQUE_MCP_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("boutique_coverage" in line for line in lines), (
        "an audit entry was written but it does not name boutique_coverage"
    )
