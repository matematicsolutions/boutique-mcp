...
Cherry-picked from dograh-hq/dograh v1.31.0 (BSD-2) via the eu-legal-mcp fleet.
"""
...
from boutique_mcp.server import INSTRUCTIONS, ToolError, mcp

SRC = (Path(__file__).parent.parent / "src" / "boutique_mcp" / "server.py").read_text(
    encoding="utf-8"
)
...
def test_instructions_only_reference_registered_tools():
    ...
    referenced = {r for r in _referenced_tool_names_in_instructions() if r.startswith("boutique_")}
...
def test_raised_error_codes_in_valid_codes():
    raised = set(re.findall(r'ToolError\(\s*\n?\s*"(\w+)"\s*,', SRC))
...
def test_no_query_content_in_audit_calls():
    """Constitution Article II: audit lines carry tool name + timing only."""
    for m in re.finditer(r"audit\.log\(([^)]*)\)", SRC):
        args = m.group(1)
        assert "query" not in args and "description" not in args and "id=" not in args, (
            f"audit.log must not receive query content: {args}"
        )
